...
while (true) {
  ...
  if (!gameStats.exploring) {
    ...
    const paths = astar(matrice, player, gameStats.startPos);
    const output = getOutput(paths[0]);
    paths.shift();
    toFollow.push(...paths);
    console.log(output);
    continue;
  ...
  const startR = Math.max(0, player.r - 2);
  const startC = Math.max(0, player.c - 2);
  for (let j = 0; j < 5; j++) {
    for (let i = 0; i < 5; i++) {
  ...
  if (tileToExplore.length === 0) {
    ...
    const paths = astar(matrice, player, gameStats.endPos);
    const output = getOutput(paths[0]);
    paths.shift();
    toFollow.push(...paths);
    gameStats.exploring = false;
    console.log(output);
    continue;
  }
